//! The single-entity-vs-separate toggle (ARCHITECTURE §6), ported from `scope.mjs`.
//!
//! What makes N CLIs ONE entity is not the harness — it is that their outputs read and write the
//! SAME collection scope on the shared store. The toggle is a scope decision owned here:
...
//!
//! Both modes live on the SAME store; the difference is purely the scope id the output node is
//! tagged with — which is the whole design claim the harness has to keep honest.
